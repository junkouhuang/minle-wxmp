<template>
  <div class="mili-page">
    <tops>
      ...
      <div slot="center" class="title">服务中心</div>
    </tops>
    <van-list
      class="mili-list"
      v-model="loading"
      ...
      @load="onLoad"
    >
      <div class="mili-item" v-for="(n, i) in list" :key="i" @click="handleLookDetail(n.applyId,n.applyStatus,n.mipuNo,n.applyMipuWays)">
        <div class="item-left">
          <img v-if="n.applyMipuCover != '' && n.applyMipuCover != null"
        ...
        <div class="item-right">
          <div class="name">
            ...
            <span class="examine-review" v-if="n.applyStatus===0">审核中<img src="./img/result.png"/></span>
            <span class="examine-ok" v-if="n.applyStatus===1">已开通<img class="point" src="./img/online.png"/></span>
          ...
          <div class="sent">
            <span>{{n.applyMipuWays | applyMipuWays}}</span>
            <span>{{n.applyMipuType | applyMipuType}}</span>
          </div>
          <div class="address">
...
export default {
  ...
  methods: {
    ...
    //查看详情
    handleLookDetail(applyId,applyStatus,mipuNo, applyMipuWays){
      if (applyStatus == 1) {
          this.$router.push({
        ...
        /*
        if (applyMipuWays == 1) {
          this.buyNow(mipuNo, '58209736479000');
        } else if (applyMipuWays == 2) {
          this.buyNow(mipuNo, '58209736479001');
        } else if (applyMipuWays == 3) {
          this.buyNow(mipuNo, '58209736479002');
        } else if (applyMipuWays == 4) {
          this.buyNow(mipuNo, '58209736479003');
        }*/
      }
    },
    async buyNow(mipuNo, commo) {
      let res = await this.$http({
        method: 'post',
        url: '/minle/mall/cart/settlement',
        data: {commodityNoList: [commo]}
      })
      if (res.status == 200) {
        this.$router.push({
          path: '/orderconfirm',
