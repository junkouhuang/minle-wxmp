<template>
  <div class="audit-page">
    ...
    <div class="mili-shell" v-if="mipuData">
      <div class="mili-item">
        <div class="item-left">
          <img :src="mipuData.applyMipuCover" alt="" />
          <div class="text-result"></div>
        </div>
        <div class="item-right">
          <p class="name">
          <span>{{mipuData.applyMipuName}}</span>
            <span class="examine-review" v-if="mipuData.applyStatus===0">米铺审核中<img src="./img/result.png"/></span>
            <span class="examine-ok" v-if="mipuData.applyStatus===1">在线米铺<img class="point" src="./img/online.png"/></span>
    ...
    <p class="under-review" v-if="mipuData && mipuData.applyStatus === 0">
      您申请的<span style="color:#1e81ef;">{{mipuData.applyMipuWays | applyMipuWays}}正在审核中</span>，平台将会在1~2工作日内完成审核。审核期间，平台工作人员将会通过电话与您取得联系，请您保持联系电话或手机处于持续开机状态，以免影响米铺审核进展。
    </p>
    <p class="under-review" v-if="mipuData && mipuData.applyStatus === 2">
      您申请的<span style="color:#ff3444;">{{mipuData.applyMipuWays | applyMipuWays}}未审核通过</span>，详细原因请查看审核结果。建议您仔细核对您的米铺申请信息，确认无误后，你可以重新提交米铺申请。
    </p>
    <p class="title" >审核结果</p>
    <p class="reason" v-if="mipuData && mipuData.applyStatus === 0">
      米铺申请正在受理审核中，非常感谢您的使用。
    </p>
    ...
    <div class="delete" @click="handleDelete(mipuData)"><span><img style="vertical-align:middle;" src="./img/shanchu.png"/></span>删除米铺</div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    async handleDelete(mipuData) {
      ...
      if(mipuData.applyStatus == 0){
        this.$toast("米铺正在审核中，暂不支持删除");
        return;
      }
      const result = await this.$dialog.confirm({
        confirmButtonColor: '#FF5555',
        title: "确认删除米铺？",
        message: "米铺删除后，数据将不可恢复？"
      });
      if (!result) return;
      const resp = await this.$http({
        method: "post",
        url: "/minle/mall/mipu/deleteMipuApply",
    ...
    .mili-item {
      ...
      .item-right {
        ...
        .sent {
          ...
          span {
            font-size: 10px;
            background-color: rgba(255, 234, 214, 1);
